...
export default function Home() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Hi Nic! How can I help you today?" }
  ]);
  ...
  // Handle errors
  const handleError = () => {
    setMessages((prevMessages) => [...prevMessages, { role: "assistant", content: "Please ask me any question. For example, What is my current savings account balance?" }]);
    setLoading(false);
    setUserInput("");
  }
  ...
  return (
    <>
      <Head>
        <title>AI Neobank Demo</title>
        <meta name="description" content="AI NeoBanking interface" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.topnav}>
        <div className={styles.navlogo}>
          <a href="/">AI NeoBank interface</a>
        </div>
            ...
            {messages.map((message, index) => {
              return (
                ...
                <div key={index} className={message.role === "user" && loading && index === messages.length - 1 ? styles.usermessagewaiting : message.role === "assistant" ? styles.apimessage : styles.usermessage}>
                  ...
                  {message.role === "assistant" ? <Image src="/openai.png" alt="AI" width="40" height="30" className={styles.boticon} priority={true} /> : <Image src="/usericon.png" alt="Me" width="30" height="30" className={styles.usericon} priority={true} />}
          ...
          <div className={styles.cloudform}>
            <form onSubmit={handleSubmit}>
              <textarea
                ...
                id="userInput"
                name="userInput"
                placeholder={loading ? "Waiting for response..." : "Type your request..."}
                value={userInput}
